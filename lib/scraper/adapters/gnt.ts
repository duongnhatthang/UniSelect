/**
 * GNT Adapter — Trường Đại học Sư phạm Nghệ thuật Trung ương Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit http://www.spnttw.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Sư phạm Nghệ thuật Trung ương Hà Nội
 * Ministry code: GNT
 * Homepage: http://www.spnttw.edu.vn/
 */
...
export const gntAdapter: ScraperAdapter = {
  id: 'GNT',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `GNT adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
