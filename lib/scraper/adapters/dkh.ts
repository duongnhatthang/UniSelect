/**
 * DKH Adapter — Trường Đại học Dược Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hup.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Dược Hà Nội
 * Ministry code: DKH
 * Homepage: https://hup.edu.vn/
 */
...
export const dkhAdapter: ScraperAdapter = {
  id: 'DKH',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DKH adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
