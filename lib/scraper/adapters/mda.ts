/**
 * MDA Adapter — Trường Đại học Mỏ Địa chất Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit http://humg.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Mỏ Địa chất Hà Nội
 * Ministry code: MDA
 * Homepage: http://humg.edu.vn/
 */
...
export const mdaAdapter: ScraperAdapter = {
  id: 'MDA',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `MDA adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
