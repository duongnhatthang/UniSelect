/**
 * TDD Adapter — Trường Đại học Thành Đô
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://thanhdo.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Thành Đô
 * Ministry code: TDD
 * Homepage: https://thanhdo.edu.vn/
 */
...
export const tddAdapter: ScraperAdapter = {
  id: 'TDD',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `TDD adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
