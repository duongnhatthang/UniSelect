/**
 * GTA Adapter — Trường Đại học Công nghệ Giao thông vận tải
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://utt.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Công nghệ Giao thông vận tải
 * Ministry code: GTA
 * Homepage: https://utt.edu.vn/
 */
...
export const gtaAdapter: ScraperAdapter = {
  id: 'GTA',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `GTA adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
