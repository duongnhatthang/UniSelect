/**
 * YTC Adapter — Trường Đại học Y tế Công cộng
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://huph.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Y tế Công cộng
 * Ministry code: YTC
 * Homepage: https://huph.edu.vn/
 */
...
export const ytcAdapter: ScraperAdapter = {
  id: 'YTC',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `YTC adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
