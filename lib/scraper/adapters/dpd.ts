/**
 * DPD Adapter — Trường Đại học Phương Đông
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://phuongdong.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Phương Đông
 * Ministry code: DPD
 * Homepage: https://phuongdong.edu.vn/
 */
...
export const dpdAdapter: ScraperAdapter = {
  id: 'DPD',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DPD adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
