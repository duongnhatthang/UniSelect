/**
 * QHE Adapter — Trường Đại học Kinh tế - ĐHQG Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://ueb.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Kinh tế - ĐHQG Hà Nội
 * Ministry code: QHE
 * Homepage: https://ueb.edu.vn/
 */
...
export const qheAdapter: ScraperAdapter = {
  id: 'QHE',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `QHE adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
