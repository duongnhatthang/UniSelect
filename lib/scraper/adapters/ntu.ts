/**
 * NTU Adapter — Trường Đại học Nguyễn Trãi
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://daihocnguyentrai.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 * Ministry code: NTU
 * Homepage: https://daihocnguyentrai.edu.vn/
 */
...
export const ntuAdapter: ScraperAdapter = {
  id: 'NTU',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `NTU adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
