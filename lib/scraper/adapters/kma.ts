/**
 * KMA Adapter — Học viện Kỹ thuật Mật mã
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://actvn.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
...
export const kmaAdapter: ScraperAdapter = {
  id: 'KMA',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `KMA adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
