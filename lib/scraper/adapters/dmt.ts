/**
 * DMT Adapter — Trường Đại học Tài nguyên và Môi trường Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hunre.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Tài nguyên và Môi trường Hà Nội
 * Ministry code: DMT
 * Homepage: https://hunre.edu.vn/
 */
...
export const dmtAdapter: ScraperAdapter = {
  id: 'DMT',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DMT adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
