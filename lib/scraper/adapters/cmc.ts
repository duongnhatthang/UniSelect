/**
 * CMC Adapter — Trường Đại học CMC
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://cmc-u.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học CMC
 * Ministry code: CMC
 * Homepage: https://cmc-u.edu.vn/
 */
...
export const cmcAdapter: ScraperAdapter = {
  id: 'CMC',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `CMC adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
