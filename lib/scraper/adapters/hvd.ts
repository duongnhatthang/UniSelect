/**
 * HVD Adapter — Học viện Dân tộc
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hvdt.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Học viện Dân tộc
 * Ministry code: HVD
 * Homepage: https://hvdt.edu.vn/
 */
...
export const hvdAdapter: ScraperAdapter = {
  id: 'HVD',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HVD adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
