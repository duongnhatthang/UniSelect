/**
 * NHH Adapter — Học viện Ngân hàng
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hvnh.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
...
export const nhhAdapter: ScraperAdapter = {
  id: 'NHH',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `NHH adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
