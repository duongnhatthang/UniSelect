/**
 * PKA Adapter — Trường Đại học Phenikaa
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://phenikaa-uni.edu.vn/vi and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Phenikaa
 * Ministry code: PKA
 * Homepage: https://phenikaa-uni.edu.vn/vi
 */
...
export const pkaAdapter: ScraperAdapter = {
  id: 'PKA',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `PKA adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
