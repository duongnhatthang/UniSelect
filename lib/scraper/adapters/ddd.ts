/**
 * DDD Adapter — Trường Đại học Đông Đô
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hdiu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Đông Đô
 * Ministry code: DDD
 * Homepage: https://hdiu.edu.vn/
 */
...
export const dddAdapter: ScraperAdapter = {
  id: 'DDD',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DDD adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
