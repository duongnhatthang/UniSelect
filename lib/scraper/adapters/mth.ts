/**
 * MTH Adapter — Trường Đại học Mỹ thuật Việt Nam
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://mythuatvietnam.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Mỹ thuật Việt Nam
 * Ministry code: MTH
 * Homepage: https://mythuatvietnam.edu.vn/
 */
...
export const mthAdapter: ScraperAdapter = {
  id: 'MTH',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `MTH adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
