/**
 * HCH Adapter — Học viện Hành chính Quốc gia
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://www1.napa.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
...
export const hchAdapter: ScraperAdapter = {
  id: 'HCH',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HCH adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
