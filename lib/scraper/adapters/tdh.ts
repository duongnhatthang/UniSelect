/**
 * TDH Adapter — Trường Đại học Sư phạm Thể dục thể thao Hà nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hupes.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Sư phạm Thể dục thể thao Hà nội
 * Ministry code: TDH
 * Homepage: https://hupes.edu.vn/
 */
...
export const tdhAdapter: ScraperAdapter = {
  id: 'TDH',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `TDH adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
