/**
 * CCM Adapter — Trường Đại học Công nghiệp Dệt may Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit http://hict.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Công nghiệp Dệt may Hà Nội
 * Ministry code: CCM
 * Homepage: http://hict.edu.vn/
 */
...
export const ccmAdapter: ScraperAdapter = {
  id: 'CCM',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `CCM adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
