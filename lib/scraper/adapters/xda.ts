/**
 * XDA Adapter — Trường Đại học Xây dựng Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://huce.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Xây dựng Hà Nội
 * Ministry code: XDA
 * Homepage: https://huce.edu.vn/
 */
...
export const xdaAdapter: ScraperAdapter = {
  id: 'XDA',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `XDA adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
