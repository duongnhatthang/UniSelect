/**
 * LNH Adapter — Trường Đại học Lâm nghiệp
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://vnuf.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Lâm nghiệp
 * Ministry code: LNH
 * Homepage: https://vnuf.edu.vn/
 */
...
export const lnhAdapter: ScraperAdapter = {
  id: 'LNH',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `LNH adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
