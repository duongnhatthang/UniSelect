/**
 * VHH Adapter — Trường Đại học Văn hóa Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://huc.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Văn hóa Hà Nội
 * Ministry code: VHH
 * Homepage: https://huc.edu.vn/
 */
...
export const vhhAdapter: ScraperAdapter = {
  id: 'VHH',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `VHH adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
