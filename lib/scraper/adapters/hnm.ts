/**
 * HNM Adapter — Trường Đại học Thủ đô Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hnmu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Thủ đô Hà Nội
 * Ministry code: HNM
 * Homepage: https://hnmu.edu.vn/
 */
...
export const hnmAdapter: ScraperAdapter = {
  id: 'HNM',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HNM adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
