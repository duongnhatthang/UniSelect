/**
 * HVN Adapter — Học viện Nông nghiệp Việt Nam
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://tuyensinh.vnua.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 * Ministry code: HVN
 * Homepage: https://tuyensinh.vnua.edu.vn/
 */
...
export const hvnAdapter: ScraperAdapter = {
  id: 'HVN',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HVN adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
