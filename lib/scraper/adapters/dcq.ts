/**
 * DCQ Adapter — Trường Đại học Công nghệ và Quản lý Hữu nghị
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://utm.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Công nghệ và Quản lý Hữu nghị
 * Ministry code: DCQ
 * Homepage: https://utm.edu.vn/
 */
...
export const dcqAdapter: ScraperAdapter = {
  id: 'DCQ',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DCQ adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
