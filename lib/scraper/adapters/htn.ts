/**
 * HTN Adapter — Học viện Thanh Thiếu niên Việt Nam
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://vya.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Học viện Thanh Thiếu niên Việt Nam
 * Ministry code: HTN
 * Homepage: https://vya.edu.vn/
 */
...
export const htnAdapter: ScraperAdapter = {
  id: 'HTN',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HTN adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
