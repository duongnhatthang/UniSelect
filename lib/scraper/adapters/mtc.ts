/**
 * MTC Adapter — Trường Đại học Mỹ thuật Công nghiệp
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://uad.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Mỹ thuật Công nghiệp
 * Ministry code: MTC
 * Homepage: https://uad.edu.vn/
 */
...
export const mtcAdapter: ScraperAdapter = {
  id: 'MTC',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `MTC adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
