/**
 * DDN Adapter — Trường Đại học Đại Nam
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://dainam.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Đại Nam
 * Ministry code: DDN
 * Homepage: https://dainam.edu.vn/
 */
...
export const ddnAdapter: ScraperAdapter = {
  id: 'DDN',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DDN adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
