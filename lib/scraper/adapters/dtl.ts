/**
 * DTL Adapter — Trường Đại học Thăng Long
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://thanglong.edu.vn/node/1 and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 * Ministry code: DTL
 * Homepage: https://thanglong.edu.vn/node/1
 */
...
export const dtlAdapter: ScraperAdapter = {
  id: 'DTL',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DTL adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
