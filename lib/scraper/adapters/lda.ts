/**
 * LDA Adapter — Trường Đại học Công đoàn
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit http://dhcd.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Công đoàn
 * Ministry code: LDA
 * Homepage: http://dhcd.edu.vn/
 */
...
export const ldaAdapter: ScraperAdapter = {
  id: 'LDA',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `LDA adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
