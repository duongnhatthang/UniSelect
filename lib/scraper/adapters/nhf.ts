/**
 * NHF Adapter — Trường Đại học Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://www.hanu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Hà Nội
 * Ministry code: NHF
 * Homepage: https://www.hanu.vn/
 */
...
export const nhfAdapter: ScraperAdapter = {
  id: 'NHF',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `NHF adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
