/**
 * NTH Adapter — Trường Đại học Ngoại thương (FTU)
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://ftu.edu.vn/ and find the cutoff scores page (likely under
...
export const nthAdapter: ScraperAdapter = {
  id: 'NTH',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `NTH adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
