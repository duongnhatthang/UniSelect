/**
 * HQT Adapter — Học viện Ngoại giao
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://www.dav.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
...
export const hqtAdapter: ScraperAdapter = {
  id: 'HQT',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HQT adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
