/**
 * TGC Adapter — Học viện Báo chí Tuyên truyền
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://daotaoajc.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 * Ministry code: TGC
 * Homepage: https://daotaoajc.edu.vn/
 */
...
export const tgcAdapter: ScraperAdapter = {
  id: 'TGC',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `TGC adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
