/**
 * NVH Adapter — Học viện Âm nhạc Quốc gia Việt Nam
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://vnam.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Học viện Âm nhạc Quốc gia Việt Nam
 * Ministry code: NVH
 * Homepage: https://vnam.edu.vn/
 */
...
export const nvhAdapter: ScraperAdapter = {
  id: 'NVH',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `NVH adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
