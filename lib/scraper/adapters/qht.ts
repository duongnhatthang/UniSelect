/**
 * QHT Adapter — Trường ĐH Khoa học Tự nhiên - ĐHQG Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hus.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường ĐH Khoa học Tự nhiên - ĐHQG Hà Nội
 * Ministry code: QHT
 * Homepage: https://hus.edu.vn/
 */
...
export const qhtAdapter: ScraperAdapter = {
  id: 'QHT',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `QHT adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
