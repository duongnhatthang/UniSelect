/**
 * DQK Adapter — Trường Đại học Kinh doanh và Công nghệ Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hubt.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Kinh doanh và Công nghệ Hà Nội
 * Ministry code: DQK
 * Homepage: https://hubt.edu.vn/
 */
...
export const dqkAdapter: ScraperAdapter = {
  id: 'DQK',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DQK adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
