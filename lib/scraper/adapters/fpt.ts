/**
 * FPT Adapter — Trường Đại học FPT
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hanoi.fpt.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 * Ministry code: FPT
 * Homepage: https://hanoi.fpt.edu.vn/
 */
...
export const fptAdapter: ScraperAdapter = {
  id: 'FPT',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `FPT adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
