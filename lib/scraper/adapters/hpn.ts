/**
 * HPN Adapter — Học viện Phụ nữ Việt Nam
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hvpnvn.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Học viện Phụ nữ Việt Nam
 * Ministry code: HPN
 * Homepage: https://hvpnvn.edu.vn/
 */
...
export const hpnAdapter: ScraperAdapter = {
  id: 'HPN',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HPN adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
