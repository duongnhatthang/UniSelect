/**
 * KCN Adapter — Trường Đại học Khoa học và Công nghệ Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://usth.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Khoa học và Công nghệ Hà Nội
 * Ministry code: KCN
 * Homepage: https://usth.edu.vn/
 */
...
export const kcnAdapter: ScraperAdapter = {
  id: 'KCN',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `KCN adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
