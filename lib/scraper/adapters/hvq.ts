/**
 * HVQ Adapter — Học viện Quản lý Giáo dục
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://www.naem.edu.vn/vi and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Học viện Quản lý Giáo dục
 * Ministry code: HVQ
 * Homepage: https://www.naem.edu.vn/vi
 */
...
export const hvqAdapter: ScraperAdapter = {
  id: 'HVQ',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HVQ adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
