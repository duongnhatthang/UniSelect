/**
 * QHI Adapter — Trường Đại học Công nghệ - ĐHQG Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://uet.vnu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Công nghệ - ĐHQG Hà Nội
 * Ministry code: QHI
 * Homepage: https://uet.vnu.edu.vn/
 */
...
export const qhiAdapter: ScraperAdapter = {
  id: 'QHI',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `QHI adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
