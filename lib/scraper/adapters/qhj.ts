/**
 * QHJ Adapter — Trường Đại học Việt Nhật - ĐHQG Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://vju.ac.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Việt Nhật - ĐHQG Hà Nội
 * Ministry code: QHJ
 * Homepage: https://vju.ac.vn/
 */
...
export const qhjAdapter: ScraperAdapter = {
  id: 'QHJ',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `QHJ adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
