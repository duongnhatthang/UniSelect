/**
 * QHY Adapter — Trường Đại học Y Dược - ĐHQG Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://ump.vnu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Y Dược - ĐHQG Hà Nội
 * Ministry code: QHY
 * Homepage: https://ump.vnu.edu.vn/
 */
...
export const qhyAdapter: ScraperAdapter = {
  id: 'QHY',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `QHY adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
