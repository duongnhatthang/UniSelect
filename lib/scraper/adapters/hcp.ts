/**
 * HCP Adapter — Học viện Chính sách và Phát triển
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://apd.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Học viện Chính sách và Phát triển
 * Ministry code: HCP
 * Homepage: https://apd.edu.vn/
 */
...
export const hcpAdapter: ScraperAdapter = {
  id: 'HCP',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HCP adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
