/**
 * QHF Adapter — Trường Đại học Ngoại ngữ - ĐHQG Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://ulis.vnu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Ngoại ngữ - ĐHQG Hà Nội
 * Ministry code: QHF
 * Homepage: https://ulis.vnu.edu.vn/
 */
...
export const qhfAdapter: ScraperAdapter = {
  id: 'QHF',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `QHF adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
