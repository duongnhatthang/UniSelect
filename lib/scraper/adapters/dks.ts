/**
 * DKS Adapter — Trường Đại học Kiểm sát Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hpu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Kiểm sát Hà Nội
 * Ministry code: DKS
 * Homepage: https://hpu.vn/
 */
...
export const dksAdapter: ScraperAdapter = {
  id: 'DKS',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DKS adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
