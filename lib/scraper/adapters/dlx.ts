/**
 * DLX Adapter — Trường Đại học Lao động Xã hội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit http://ulsa.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Lao động Xã hội
 * Ministry code: DLX
 * Homepage: http://ulsa.edu.vn/
 */
...
export const dlxAdapter: ScraperAdapter = {
  id: 'DLX',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DLX adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
