/**
 * SKD Adapter — Trường Đại học Sân khấu Điện ảnh
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://skda.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Sân khấu Điện ảnh
 * Ministry code: SKD
 * Homepage: https://skda.edu.vn/
 */
...
export const skdAdapter: ScraperAdapter = {
  id: 'SKD',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `SKD adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
