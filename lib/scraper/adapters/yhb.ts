/**
 * YHB Adapter — Trường Đại học Y Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hmu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Y Hà Nội
 * Ministry code: YHB
 * Homepage: https://hmu.edu.vn/
 */
...
export const yhbAdapter: ScraperAdapter = {
  id: 'YHB',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `YHB adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
