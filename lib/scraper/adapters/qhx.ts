/**
 * QHX Adapter — Trường ĐH Khoa học Xã hội và Nhân văn - ĐHQG Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://ussh.vnu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường ĐH Khoa học Xã hội và Nhân văn - ĐHQG Hà Nội
 * Ministry code: QHX
 * Homepage: https://ussh.vnu.edu.vn/
 */
...
export const qhxAdapter: ScraperAdapter = {
  id: 'QHX',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `QHX adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
