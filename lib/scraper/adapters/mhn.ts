/**
 * MHN Adapter — Trường Đại học Mở Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hou.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
...
export const mhnAdapter: ScraperAdapter = {
  id: 'MHN',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `MHN adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
