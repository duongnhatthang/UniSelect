/**
 * DCN Adapter — Trường Đại học Công nghiệp Hà Nội (HaUI)
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://www.haui.edu.vn/ and find the cutoff scores page (likely under
 ...
 * Selection rationale: High enrollment industrial university, established web presence, likely well-structured cutoff tables
 */
...
export const dcnAdapter: ScraperAdapter = {
  ...
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DCN adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
