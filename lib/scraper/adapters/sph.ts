/**
 * SPH Adapter -- Truong Dai hoc Su pham Ha Noi (HNUE)
 *
 * Verified: 2026-03-18 (candidate, static HTML expected)
 * Rendering: Static HTML
 * URL: https://hnue.edu.vn/Tuyensinh--Dai-hoc-chinh-quy/diem-trung-tuyen
 * Pattern: Static HTML adapter (cheerio) -- see htc.ts for reference pattern
 *
 * University: Truong Dai hoc Su pham Ha Noi
 * Ministry code: SPH
 * Homepage: https://hnue.edu.vn/
 */
...
export const sphAdapter: ScraperAdapter = {
  id: 'SPH',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `SPH adapter returned 0 rows -- possible JS rendering or layout change at ${url}`
      );
    }
