/**
 * HBU Adapter — Trường Đại học Hòa Bình
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://daihochoabinh.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Hòa Bình
 * Ministry code: HBU
 * Homepage: https://daihochoabinh.edu.vn/
 */
...
export const hbuAdapter: ScraperAdapter = {
  id: 'HBU',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HBU adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
