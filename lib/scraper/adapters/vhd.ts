/**
 * VHD Adapter — Trường Đại học Công nghiệp Việt Hung
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://viu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Công nghiệp Việt Hung
 * Ministry code: VHD
 * Homepage: https://viu.edu.vn/
 */
...
export const vhdAdapter: ScraperAdapter = {
  id: 'VHD',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `VHD adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
