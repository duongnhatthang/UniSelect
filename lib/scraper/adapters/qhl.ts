/**
 * QHL Adapter — Trường Đại học Luật - ĐHQG Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://law.vnu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Luật - ĐHQG Hà Nội
 * Ministry code: QHL
 * Homepage: https://law.vnu.edu.vn/
 */
...
export const qhlAdapter: ScraperAdapter = {
  id: 'QHL',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `QHL adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
