/**
 * HYD Adapter — Học viện Y Dược học cổ truyền Việt Nam
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit http://vutm.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Học viện Y Dược học cổ truyền Việt Nam
 * Ministry code: HYD
 * Homepage: http://vutm.edu.vn/
 */
...
export const hydAdapter: ScraperAdapter = {
  id: 'HYD',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HYD adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
