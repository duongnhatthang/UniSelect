/**
 * QHS Adapter — Trường ĐH Giáo dục - ĐHQG Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://education.vnu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường ĐH Giáo dục - ĐHQG Hà Nội
 * Ministry code: QHS
 * Homepage: https://education.vnu.edu.vn/
 */
...
export const qhsAdapter: ScraperAdapter = {
  id: 'QHS',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `QHS adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
