/**
 * TMA Adapter — Trường Đại học Thương mại
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://tmu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Thương mại
 * Ministry code: TMA
 * Homepage: https://tmu.edu.vn/
 */
...
export const tmaAdapter: ScraperAdapter = {
  id: 'TMA',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `TMA adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
