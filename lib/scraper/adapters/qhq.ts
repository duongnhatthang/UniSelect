/**
 * QHQ Adapter — Trường Quốc tế - ĐHQG Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://tuyensinh.vnuis.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Quốc tế - ĐHQG Hà Nội
 * Ministry code: QHQ
 * Homepage: https://tuyensinh.vnuis.edu.vn/
 */
...
export const qhqAdapter: ScraperAdapter = {
  id: 'QHQ',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `QHQ adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
