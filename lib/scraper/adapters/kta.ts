/**
 * KTA Adapter — Trường Đại học Kiến trúc Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hau.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Kiến trúc Hà Nội
 * Ministry code: KTA
 * Homepage: https://hau.edu.vn/
 */
...
export const ktaAdapter: ScraperAdapter = {
  id: 'KTA',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `KTA adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
