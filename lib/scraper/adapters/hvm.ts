/**
 * HVM Adapter — Học viện Múa Việt Nam
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://www.vnad.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Học viện Múa Việt Nam
 * Ministry code: HVM
 * Homepage: https://www.vnad.edu.vn/
 */
...
export const hvmAdapter: ScraperAdapter = {
  id: 'HVM',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HVM adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
