/**
 * DNV Adapter — Trường Đại học Nội vụ Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://huha.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Nội vụ Hà Nội
 * Ministry code: DNV
 * Homepage: https://huha.edu.vn/
 */
...
export const dnvAdapter: ScraperAdapter = {
  id: 'DNV',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DNV adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
