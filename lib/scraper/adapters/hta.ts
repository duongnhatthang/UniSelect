/**
 * HTA Adapter — Học viện Tòa án
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit http://hocvientoaan.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Học viện Tòa án
 * Ministry code: HTA
 * Homepage: http://hocvientoaan.edu.vn/
 */
...
export const htaAdapter: ScraperAdapter = {
  id: 'HTA',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `HTA adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
