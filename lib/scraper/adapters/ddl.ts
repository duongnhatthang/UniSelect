/**
 * DDL Adapter — Trường Đại học Điện lực
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://epu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Điện lực
 * Ministry code: DDL
 * Homepage: https://epu.edu.vn/
 */
...
export const ddlAdapter: ScraperAdapter = {
  id: 'DDL',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DDL adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
