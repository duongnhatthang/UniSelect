/**
 * LPH Adapter — Trường Đại học Luật Hà Nội
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://hlu.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
 ...
 *
 * University: Trường Đại học Luật Hà Nội
 * Ministry code: LPH
 * Homepage: https://hlu.edu.vn/
 */
...
export const lphAdapter: ScraperAdapter = {
  id: 'LPH',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `LPH adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
