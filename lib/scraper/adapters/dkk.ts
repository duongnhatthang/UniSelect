/**
 * DKK Adapter — Trường Đại học Kinh tế Kỹ thuật Công nghiệp
 *
 * TODO: Before setting static_verified: true in scrapers.json:
 * 1. Visit https://uneti.edu.vn/ and find the cutoff scores page (Tuyển sinh -> Điểm chuẩn)
...
export const dkkAdapter: ScraperAdapter = {
  id: 'DKK',
  async scrape(url: string): Promise<RawRow[]> {
    ...
    if (rows.length === 0) {
      throw new Error(
        `DKK adapter returned 0 rows — possible JS rendering or layout change at ${url}`
      );
    }
